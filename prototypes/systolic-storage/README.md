...
| thick 3T gain cell, bank of 16×16 / 32×32 / 128×32 | 10.0 / 6.0 / 4.4 | ≥ 3.1 ms at ff/85 °C; 12 ms at tt | anything that dies within a frame's worth of lines; resident data with 0.1–0.4 % refresh |
...
| sequencer programme (4×64×16), weights, next-frame config | 0.8–4 kbit | resident | thick with 0.1–0.4 % of one port for refresh (`results/resident.txt`) |
...
| D1 8×16 latch RF in every PE | 18 | 2,304 bits, 18 ports | weights and coefficients in every PE (8×8 product), per-PE merging. Too small for any line buffer |
| **D2** edge column, 2 thick banks of 128×32 | **31** | 8,192 bits, 2 ports | line filters, Ethernet line buffer up to 5 lines per packet, programme store, next-frame config, with no refresh except for resident data |
| D2s the same with 2 SRAM `1P_256x16` | 28 | 8,192 bits, 2 ports | the same, no refresh, 3 fewer PEs |
| **D3** graded: thin 32×16 bank per 4 PEs, 2 thick banks at the edge | 26 | 11,776 bits, 9 ports | as D2, plus bandwidth local to the PEs (a 16-PE merge at 4 words per cycle): free at 27 °C, 10 % of each local bank's port for refresh at tt/85 °C, impossible at ff/85 °C |
| D4 the array as its own delay line | 40 | 32 bits per PE given up | short buffers (a 64-byte packet costs 16 PEs) |

The honest reading:
- **D2 against D2s:** the gain-cell edge column buys about 10 % more PEs than SRAM for the same
  bits. It costs refresh logic that only resident data uses.
- **D3 is the only design whose bandwidth grows with the array.** Its thin banks depend on
  temperature, so it needs a schedule for each temperature bin.
- **D1 is dominated** unless weights must be read every cycle in every PE.
- **None of the designs holds the 20-lines-per-packet buffer** (9,472 bits) inside 200k µm².
...
(`results/mixes.txt`). The all-thick mix never needs a refresh.

| trace | all thick | tt/27 °C | tt/85 °C | ff/85 °C |
|---|---|---|---|---|
| seq-regs (13 rows × 12 bit) | 2,338 µm² | 0+13, 0 % | 0+13, 1.2 % | 1+12, 6.7 % |
| linebuf-P5 (117 × 32) | 18,991 | 0+117, 2.6 %, −16 % area | 68+49, 7.4 % | 89+28, 7.4 % |
| linebuf-P20 (296 × 32) | 44,170 | 0+296, 7.7 %, −18 % | 224+72, 7.1 % | 260+36, 6.0 % |
| vfir2 (64 × 32) | 11,535 | 0+64, **0 %**, −15 % | 44+20, 9.7 % | all thick |
| vfir3 (128 × 32) | 20,538 | 0+128, 3.5 %, −16 % | 112+16, 8.1 % | all thick |

Entries are thick+thin rows. Where no area change is given, the mix saves under 7 % against all
thick.

- **Liveness pays against a blind controller.** A controller that refreshes every row of the same
  mix does 1.6–2.0× the operations on the video traces at 27 °C, and 1.2–25× hot; the gap grows
  with temperature. For linebuf-P5 at ff/85 °C that is 65,367 operations against 1,039,392.
...
- **The thin/thick mix is a second-order knob.** It saves at most 18 %, and only at room
  temperature: the cells differ by 2.20 against 2.89 µm², and periphery and Berger columns dilute
...
- **Refresh without a spare cycle:** the 3T cell's separate read and write ports would let a
  ...
  modelled.
- **The allocator is greedy.** Its only lower bound (unlimited thick rows) is trivially zero
  here, so a better bound is needed.
