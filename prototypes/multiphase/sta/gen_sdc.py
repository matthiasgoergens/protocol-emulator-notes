#!/usr/bin/env python3
"""Generate an SDC file for a given core period (ns)."""
import sys
...
sdc = f"""# Four-phase clock SDC, core period = {period} ns ({1000.0/period:.4f} MHz)
...
create_clock -name ph0 -period {period} -waveform {{0 {period/2:.6f}}} [get_ports ph0]
create_clock -name ph1 -period {period} -waveform {{{q:.6f} {q+period/2:.6f}}} [get_ports ph1]
create_clock -name ph2 -period {period} -waveform {{{2*q:.6f} {2*q+period/2:.6f}}} [get_ports ph2]
create_clock -name ph3 -period {period} -waveform {{{3*q:.6f} {3*q+period/2:.6f}}} [get_ports ph3]
