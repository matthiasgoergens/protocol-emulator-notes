    ...
    if mode == "palette":
        ...
        for hue in range(16):
            for luma in range(11):
                line = 30 + (luma * 224) // 11 + 10 - 22
                x0 = 768 * (0.5 + 0.1 + (hue * 16 + 8) * 11 / 3013) / 1.0
                # pixel -> decoder column: pixel px starts at clock 722 + 11 px; decoder spans clocks 624..3637
                col = int(((722 + 11 * (hue * 16 + 8)) - 10.9e-6 * FS) / (52.6e-6 * FS) * w)
        ...
        paths = sorted(pathlib.Path("out").glob("frame_*.bin"))
        frames = []
        for p in paths:
            img = decode(p)
        ...
        frames[0].save("out/game.gif", save_all=True, append_images=frames[1:], duration=33, loop=0)
