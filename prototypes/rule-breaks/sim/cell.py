...
GDL = E("GDL", "0.30")
WMS = E("WMS", "0.30")
WMR = E("WMR", "0.30")   # rule-breaks: read transistor width (0.20 when strips are 0.20 wide)
...
def read_devs(i, sn, bar, rwl):
    ...
    return (f"XMS{i} mid{i} {sn} {bar} 0 {ms} w={WMS}u l={l}u\n"
            f"XMR{i} rbl {rwl} mid{i} 0 sg13_lv_nmos w={WMR}u l=0.13u" + gd)
...
def hold():
    print(f"hold: WMS={WMS} WMR={WMR} MW={MW} MS={MS} VLO={VLO} VHI={VHI} VBAR={VBAR} WBLH={WBLH} VWWL={VWWL} "
...
def read():
    ...
    print(f"read: WMS={WMS} WMR={WMR} MW={MW} MS={MS} VLO={VLO} VBAR={VBAR} RBAR={RBAR} N={N} CBL={CBL} SN1={SN1}; "
