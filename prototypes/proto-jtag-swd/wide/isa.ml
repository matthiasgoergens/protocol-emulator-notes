(* VARIANT of ../../deadline-sequencer/isa.ml: pc and all address fields widened from 6 to 8 bits
   (WAITP fail[7:0], JMP/JNZ addr[7:0]). Everything else is unchanged; see ../README.md. *)
...
let n_threads = 4
let pc_bits = 8   (* VARIANT: 6 in the stock ISA; 256 words per thread, the planned 1024x16 store *)
...
let waitp ~pin ~value ~fail = enc WAITP ((pin lsl 9) lor (value lsl 8) lor (fail land (prog_len - 1)))
...
let jmp a = enc JMP (a land (prog_len - 1))
let jnz a = enc JNZ (a land (prog_len - 1))
let out = enc OUT 0
let in_ = enc IN 0
let halt = enc HALT 0
...
let step st ~(mem : int array array) ~pin_in ~host_in ~host_in_valid =
  ...
  let addr6 = instr land (prog_len - 1) in   (* VARIANT: 8-bit address fields *)
