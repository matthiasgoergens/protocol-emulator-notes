...
vwbl wbl 0 pwl(0 {VDD if bit else 0} 30n {VDD if bit else 0} 31n {0 if bit else VDD})
...
meas tran tlost when v(sn)={0.45 if bit else 0.35} {'fall' if bit else 'rise'}=1
.endc
.end
"""
...
for wt, corner, temp, bit in cases:
    ...
    rows.append((wt, corner, temp, g("vw"), g("v1u"), g("v100u"), g("v1m"), g("v10m"), g("tlost")))
    r = rows[-1]
    fmt = lambda x: "  n/a " if x is None else f"{x:6.3f}"
    t = "> 50 ms" if r[8] is None else (f"{r[8]*1e6:9.1f} us")
    print(f"{wt:8s} {corner:7s} {temp:3d}C  stored {bit}  written {fmt(r[3])} V  after 1us {fmt(r[4])}  100us {fmt(r[5])}  1ms {fmt(r[6])}  10ms {fmt(r[7])}  lost at {t}", flush=True)
    if r[3] is None:
