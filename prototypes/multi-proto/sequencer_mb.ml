(* Mailbox variant of the deadline sequencer RTL. See isa_mb.ml for the semantics this must match
   ...
   inbox overwrites the oldest entry. *)
open Hardcaml
open Signal
...
let create ?(mutant = `None) ~(c : Isa_mb.cfg) ~clock ~clear ~imem_data ~pin_in ~host_in ~host_in_valid
  ...
  let full i = match mutant with `Full_never -> gnd | `None -> counts.(i).value ==:. d in
        ...
        proc
          [ when_ (push.value &: mine)
              ...
              ; (match mutant with
                 ...
                 | `None -> counts.(i) <-- counts.(i).value +:. 1) ]
  ...
  , port_out_data.value, port_out_valid.value, port_in_ready.value, counts_out )
...
let circuit ?mutant (c : Isa_mb.cfg) =
  ...
  let imem_addr, pin_out, pin_oe, host_out, host_out_valid, host_in_ready, pcs, pod, pov, pir, cnts =
