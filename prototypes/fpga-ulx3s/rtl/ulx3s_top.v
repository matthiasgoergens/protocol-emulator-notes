...
//             4 USB configured, 5 PLL locked, 6 USB address nonzero, 7 flash route on (or
//             the flash interlock tripped)
//   btn[1] (FIRE1)  reset
//
...
`endif
  ...
  // Chip select stays high unless the host routes the sequencer to the flash (ctrl bit 0); the
  // runner refuses flash-routed programmes that would send a non-read command, and emu_core's
  // interlock cuts off any that get through. USRMCLKTS tristates the clock while the route is
  // off. It is driven from a register, not a constant: Lattice's sysCONFIG guide asks for a signal.
  USRMCLK usrmclk (.USRMCLKI(spi_sclk), .USRMCLKTS(~ctrl[0]));
  ...
  assign led = {ctrl[0] | flash_blocked, usb_status_s2[6:0] != 7'd0, locked, usb_status_s2[7], activity, trace_ovf, running, heartbeat[25]};
endmodule
